...
【1.市场环境】先用表格列出三大指数当日行情（指数名称、收盘点位、涨跌幅、成交额），再判断市场整体环境（强势/震荡/弱势/退潮），给出操作建议（主动进攻/精选参与/控制仓位/观望）
...
【4.核心锚点个股】用表格列出5-8只核心个股（排除新股/次新股，涨幅超过100%的视为新股不得列入），列：股票、板块、涨跌幅(%)、成交额(亿)、定位（情绪龙头/趋势中军/补涨标的/跟风后排）
【5.当前情绪】只输出阶段名称和一句话定性描述，阶段必须使用 analysis 数据中 emotion.phase 的值（冰点/调整/修复/主升/高潮），不得输出评分维度、打分逻辑等判断过程
【6.主线持续性评估】对每条主线分别评估：产业逻辑（✅/⚠️）、事件催化（✅/⚠️）、资金合力（✅/⚠️），给出持续性等级（强/较强/一般/弱），指出风险触发条件
